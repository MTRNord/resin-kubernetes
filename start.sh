...
if [ "$LOG" == "file" ]
then
	rce -d -e lxc --bip=${FLANNEL_SUBNET} --mtu=${FLANNEL_MTU} $DOCKER_DAEMON_ARGS &>/var/log/rce.log &
else
	rce -d -e lxc --bip=${FLANNEL_SUBNET} --mtu=${FLANNEL_MTU} $DOCKER_DAEMON_ARGS &
fi
(( timeout = 60 + SECONDS ))
until rce info >/dev/null 2>&1
do
	if (( SECONDS >= timeout )); then
		echo 'Timed out trying to connect to internal rce host.' >&2
		break
	fi
	sleep 1
done
